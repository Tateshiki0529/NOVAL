...
mixhostは収容サーバーによってMariaDB 10.11または10.5が提供される。NOVALの対象はMariaDB 10.11以上であるため、契約・移設前に収容サーバーのVersionを確認する。
...
```text
/home/account/noval/current/public
```
...
```text
/home/account/noval/current/       # Application、vendor、storage
...
- maintenance file: `noval/current/storage/framework/maintenance.php`
- Composer autoloader: `noval/current/vendor/autoload.php`
- Laravel bootstrap: `noval/current/bootstrap/app.php`
...
```text
* * * * * cd /absolute/path/noval/current && /absolute/path/php8.3 artisan schedule:run >> /dev/null 2>&1
```
