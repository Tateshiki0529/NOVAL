...
PostgreSQLを仮定し、可変構造データには`jsonb`を使用する。
...
実装言語は未決である。TypeScriptを採用する場合、ドメインロジックをWebフレームワークから独立させる。
...
Web、API、CSV、Webhookは同じapplication serviceを呼び出す。

```text
Web Form ─┐
REST API ─┼─→ CreateRecordService → Normalize → Validate → Store
CSV ──────┤
Webhook ──┘
```
...
4. 認証方式
5. ホスティング先とAsset保存先
6. `number`およびmeasurementの精度保証
7. JavaScript safe integerを超える整数の表現
8. decimalをJSON numberまたは文字列のどちらで保存するか
9. ProtocolおよびRecordの最大サイズ
10. Field IDの確定規則と予約語一覧
11. Protocol fork元および作者帰属の表示方針
12. 古いRecordを編集する際のProtocol Version選択方針
13. Matrixの初期集計に絞り込み条件を含めるか
14. 公開LogBookにおけるRecord表示形式
15. Categoryの正式名称
16. Open Recordの正式名称
17. Open RecordのbodyでMarkdownを許可するか
18. Open Recordのbody最大長
19. Open RecordへAssetを追加する時期と参照形式
20. Category visibilityをLogBook公開へ継承するか
21. withdrawn Protocol Versionの導入時期
22. Protocol Recordのextensionsカラムを物理的に完全削除するか、将来互換の予約領域としてDBだけに残すか
23. 将来の添付込みZIPコンテナの拡張子、MIME type、manifest仕様
24. NOVA JSON交換形式の初回`formatVersion`
...
1. 数値表現と精度保証
2. 実装言語、フレームワーク、DB
3. Protocolの保存SchemaとField ID規則
4. Record Revisionの編集時Version方針
5. CategoryおよびOpen Recordの正式名称と公開範囲
6. 認証、公開範囲、Asset保存
7. NOVA JSON交換形式の初回formatVersion
8. API契約
9. 画面構成
10. Matrixの詳細
