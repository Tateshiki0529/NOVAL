...
> **NOVAL — Notation for Open and Versatile Archives Layer**  
...
本書は、NOVALのプロダクト思想、用語、情報モデル、Category、Protocol、LogBook、Protocol Record、Open Record、バリデーション、履歴管理、公開機能、Matrix、API、データベース、および段階的MVPの実装範囲を定義する。

本書では、会話の中で合意済みの事項と、現時点での設計提案、今後決定が必要な事項を区別して記載する。

- **必須**: NOVALとして満たす必要がある要件
- **推奨**: 現時点で採用を推奨する設計
...
NOVALは、ユーザーが記録対象の構造とルールを自ら定義して構造化Recordを保存でき、Protocolを必要としない自由記述も残せる、一般公開型の汎用アーカイブサービスである。
...
- NOVAL独自のValidation Rule配列を重複して持たない。
- UI表示設定、入力Widget、単位、アイコンなどはNOVAL metadataとして分離する。
- attachment、imageなど外部状態を必要とする検証はNOVAL固有Validatorで扱う。
- `format`はNOVALサーバー側でassertionとして有効化する。
...
- NOVALは将来的に推奨Version bumpを提示できるが、MVPでは自動互換性判定を行わない。
...
| 単位付き数値 | 構造化object＋NOVAL metadata |
...
原則としてJSON Schema keywordをそのまま使用する。`unit`はNOVAL metadataと、measurement objectのSchema制約を組み合わせて表現する。
...
- 将来自動注入する場合は、NOVAL metadataで明示的に有効化する。
...
interface CorrelationConstraint {
  ...
  expressionLanguage: "noval-expr-v1";
...
- `rule`は違反したSchemaまたはNOVALルールを表す。
...
- `receivedAt`: NOVALがRecordを受信した日時
...
NOVAL運営側は、Protocolの仕組みと「すべてを同じ仕組みで記録できる」ことを示す作例を提供する。
...
- 拡張子、Content-Type、ファイル名だけを信用せず、構文解析後の`$noval`とEnvelopeを検証する。
- 未知の`$noval`とformatVersionを推測処理せず拒否する。
...
NOVALは、Protocol、LogBook、Protocol Record、Open Recordおよび複数データのArchiveに対して、JSONベースの独自エクスポート形式を定義する。NOVAL本体のVersionとエクスポート形式の`formatVersion`は独立してVersion管理する。

### 24.1 ファイル種別

| 拡張子 | `$noval` | 内容 | MIME type |
| --- | --- | --- | --- |
| `.nvp` | `protocol` | NOVAL Protocol | `application/vnd.noval.protocol+json` |
| `.nvl` | `logbook` | NOVAL LogBook | `application/vnd.noval.logbook+json` |
| `.nvr` | `record` | NOVAL Protocol Record | `application/vnd.noval.record+json` |
| `.nvo` | `open-record` | NOVAL Open Record | `application/vnd.noval.open-record+json` |
| `.nva` | `archive` | NOVAL JSON Archive | `application/vnd.noval.archive+json` |

拡張子とMIME typeは判定の補助情報とし、ファイル本文の`$noval`を正とする。拡張子から想定した種別と`$noval`が異なる場合、汎用Importerは本文に従って種別を判定したうえで`file_extension_mismatch` Warningを表示する。特定種別だけを受け付けるAPIでは`file_type_mismatch` Errorとして拒否する。
...
interface NovalContentHash {
  algorithm: "sha-256";
  canonicalization: string;
  value: string;
}

interface NovalExportEnvelope<TKind extends NovalFileKind, TData> {
  $noval: TKind;
...
- `generator`: 出力した実装名とNOVAL本体Version
- `contentHash`: 改ざん・破損検知用hash
- `sourceInstance`: 出力元Instanceの識別情報
- `metadata`: 交換形式の意味を変更しない補助情報

`generator.version`はNOVAL本体または生成実装のVersionであり、`formatVersion`とは連動させない。`contentHash`は`$noval`、`formatVersion`、`data`を規定の方法でcanonicalizeした値を対象とし、`exportedAt`、generator、sourceInstance、metadataおよびcontentHash自身は対象外とする。

### 24.3 Versionと安全な判定

- `formatVersion`は`MAJOR.MINOR.PATCH`形式とし、エクスポート形式の変更だけで更新する。
- NOVAL本体Versionが更新されても、交換形式が変わらなければformatVersionを変更しない。
- ImporterはJSONを構文解析した後、最初にEnvelopeを検証する。
- 未対応の`$noval`を推測または別形式として処理せず、`unsupported_noval_file_type`で拒否する。
...
- Envelope検証後に、`$noval`ごとのdata Schemaで全件検証する。
...
```ts
interface NovalProtocolExportData {
  protocol: Protocol;
...
```ts
interface NovalLogBookExportData {
...
```ts
interface NovalRecordExportData {
...
```ts
interface NovalOpenRecordExportData {
...
`.nva`は複数種別のNOVALデータをまとめるJSON Archiveである。

```ts
interface NovalArchiveExportData {
  entries: NovalArchiveEntry[];
  assets?: AssetManifestEntry[];
}

interface NovalArchiveEntry {
  id: string;
  $noval: Exclude<NovalFileKind, "archive">;
  formatVersion: string;
  data: JsonValue;
  contentHash?: NovalContentHash;
}
...
Export APIは種別に対応するMIME typeと`Content-Disposition`を返す。Import APIはEnvelopeの`$noval`からImporterを選択し、検証完了前にDB、Asset、外部参照を変更しない。
...
- 各拡張子、`$noval`、MIME typeの対応
- 拡張子と`$noval`不一致時に本文を正として判定すること
- 種別限定APIが不一致を拒否すること
- 未対応`$noval`を安全に拒否すること
- 未対応formatVersionをbest effortで処理しないこと
- NOVAL本体VersionとformatVersionが独立していること
...
22. NOVAL JSON交換形式の初回`formatVersion`
...
5. 認証、公開範囲、Asset保存
6. NOVAL JSON交換形式の初回formatVersion
7. API契約
8. 画面構成
9. Matrixの詳細
...
```json
{
  ...
  "schema": {
    ...
    "$id": "urn:noval:protocol:fuel:1.2.0",
    "type": "object",
    "properties": {
      "odometer": {
        "title": "走行距離",
